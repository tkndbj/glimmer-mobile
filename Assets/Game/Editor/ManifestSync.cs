        ...
        static string Serialise(ManifestDto manifest)
        {
            ...
            if (companions.Length > 0)
            {
                ...
                for (int i = 0; i < companions.Length; i++)
                {
                    ...
                    // The grove's facts about this companion's art, generated by
                    // Tools/grove_art_facts.py. Printed whether or not they are set, so the
                    // round trip below can never mistake "never generated" for "lost".
                    sb.AppendLine($"      \"groveW\": {c.groveW},");
                    sb.AppendLine($"      \"groveH\": {c.groveH},");
                    sb.AppendLine($"      \"groveHit\": \"{c.groveHit ?? string.Empty}\"");
        ...
        static bool SurvivesRoundTrip(ManifestDto before, string text, out string lost)
        {
            ...
            for (int i = 0; i < Count(before.companions); i++)
            {
                ...
                if (a.id != b.id || a.portrait != b.portrait || a.animated != b.animated ||
                    ...
                    a.groveW != b.groveW || a.groveH != b.groveH ||
                    (a.groveHit ?? string.Empty) != (b.groveHit ?? string.Empty))
                {
