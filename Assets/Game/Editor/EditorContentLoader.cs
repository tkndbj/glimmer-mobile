...
namespace GlimmerGrove.EditorTools
{
    ...
    public static class EditorContentLoader
    {
        public static EditorContent Load()
        {
            ...
            // The roster, published exactly as GameContent.Publish does at boot. Without this
            // the Editor would validate and pack the *built-in fallback* five companions rather
            // than the thirty-one the manifest ships.
            AvatarCatalog.Publish(result.Catalog.Index.Companions);

            return new EditorContent(result.Catalog, problems);
        }
    }
}
