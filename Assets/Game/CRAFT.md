...
- **The map belongs to the chapter's ordinal inside its own mode.** Every mode's first chapter draws
  ...
  5, 6, 4, 4), because `make_chapter_art.py` scales a source to *whole* strips and a count that leaves it
  narrower than 1080 stretches the map sideways. **Two of the six are `|`-joined pairs of 4:3 boards**
  rather than one tall ribbon (`map5`, the lava crag; `map6`, the wasteland mesas) - a ribbon is rare in
  these packs and a board pack is the common shape, so joining two doubles the pool of usable sources.
  Both are cut at four: three cannot seat ten nodes and six throws 60% of the painting's width away.
...
**A drawn effect reel has two failure modes a contact sheet shows and no gate can.** Both were met
cutting the sixth chapter's, and both are about the *mask* rather than about the colour:

- **Turbulence has to vary in both axes.** The anvil's front (`make_siege_art.heavefront`) is a wall of
  driven dust, and the first cut built its depth out of `sin(x/a) * sin(x/b)` terms - every one a
  function of *x* alone. What came out was a picket of vertical spikes: frost, which is the one thing it
  may not read as, because frost is the charm standing beside it (`stillwave`). No choice of frequency
  escapes it; the lobes have to be sheared by the *other* axis before they are dust.
- **A drawn bolt has to be measured against its own frame, not looked at.** The gorgon's ray
