    ...
    public sealed class ChapterModeTests
    {
        ...
        static LevelDefinition Prism(string id)
        {
            Assert.IsTrue(ProtoGrid.TryRead(new[] { "Rg@", "rrb", "b.g" }, 3, 3,
                                            PrismLayout.Letters, out var grid, out string bad), bad);

            var rules = new PrismRules(new PrismLayout(grid, 0), 0);
        ...
        public void AChapterOfOneModeDerivesThatMode()
        {
            Assert.IsTrue(ChapterModeValidator.TryDerive(
                new List<LevelDefinition> { Prism("t_a"), Prism("t_b") }, out var mode));
            Assert.AreEqual(GameMode.Prism, mode);
        ...
        public void AChapterOfTwoModesDerivesNothing()
        {
            ...
            Assert.IsFalse(ChapterModeValidator.TryDerive(
                new List<LevelDefinition> { Glade("t_a"), Prism("t_b") }, out _));
        }
        ...
        public void AnEmptyChapterDerivesNothingAndIsNotAnError()
        {
            ...
            Assert.IsFalse(ChapterModeValidator.TryDisagreement(
                Chapter, GameMode.Prism, new List<LevelDefinition>(), out _));
            Assert.IsFalse(ChapterModeValidator.TryDisagreement(Chapter, GameMode.Prism, null, out _));
        }
        ...
        {
            Assert.IsFalse(ChapterModeValidator.TryDisagreement(
                Chapter, GameMode.Prism,
                new List<LevelDefinition> { Prism("t_a"), Prism("t_b") }, out _));
        ...
        {
            // Prismvale levels in a chapter the manifest never labelled: the default is the classic
            ...
            Assert.IsTrue(ChapterModeValidator.TryDisagreement(
                ...
                new List<LevelDefinition> { Prism("t_a"), Prism("t_b") }, out var issue));

            Assert.AreEqual(LevelIssueSeverity.Error, issue.Severity);
            StringAssert.Contains("prism", issue.Message);
        ...
        {
            foreach (var claimed in new[] { GameMode.Glade, GameMode.Prism })
            {
                Assert.IsTrue(ChapterModeValidator.TryDisagreement(
                    Chapter, claimed,
                    new List<LevelDefinition> { Glade("t_a"), Prism("t_b") }, out var issue),
        ...
        public void TheMessageNamesBothModesItFound()
        {
            ...
            Assert.IsTrue(ChapterModeValidator.TryDisagreement(
                Chapter, GameMode.Glade,
                new List<LevelDefinition> { Glade("t_a"), Glade("t_b"), Prism("t_c") },
                out var issue));

            StringAssert.Contains(GameMode.Glade.Value, issue.Message);
            StringAssert.Contains(GameMode.Prism.Value, issue.Message);
        }
    }
}
