...
namespace GlimmerGrove.Content
{
    ...
    public sealed class ManifestCompanionDto
    {
        ...
        /// <summary>
        /// The pixel size of the art the grove stands this companion on — the critter frames
        /// when <see cref="animated"/> names them, the portrait otherwise — and a hit mask over
        /// it (<c>GroveHitMask</c>). <b>Generated by <c>Tools/grove_art_facts.py</c>, never
        /// typed</b>, for <c>HomesteadPieceDto.w</c>'s reason. Zero and empty in a manifest
        /// written before they existed, in which case the sprite is measured and the box is
        /// the target. <c>ManifestSync</c> writes them back and its round trip proves it.
        /// </summary>
        public int groveW;
        public int groveH;
        public string groveHit;
    ...
    public sealed class GroveFloorDto
    {
        ...
        /// <summary>
        /// How many tiles the hall covers from <see cref="hallTile"/>, its back corner. Zero or
        /// absent means one. A fact about the floor rather than the dwelling — see
        /// <c>GroveFloor.HallFootprint</c> for why buying a bigger home must never take ground.
        /// </summary>
        public int hallCols;
        public int hallRows;
    ...
    public sealed class GroveRegionDto
    {
        ...
        /// <summary>
        /// Credits that buy this region. <b>Zero means it is not sold for credits</b> — either
        /// free from the first launch, or priced in gems by <see cref="gems"/>.
        ///
        /// Zero rather than a flag, because <c>JsonUtility</c> writes a zero into every field an
        /// older file never had — so "absent" and "not for credits" have to be the same fact.
        ...
        /// <summary>
        /// Gems that buy this region instead, for the premium end of the floor. Zero means it is
        /// not sold for gems, on <see cref="cost"/>'s reasoning and for its reason.
        ///
        /// <para>
        /// <b>A region carries one price or the other, never both</b>, and
        /// <c>ContentValidation</c> errors on a region carrying two — a control labelled with a
        /// price has to charge that price, and a thing with two prices cannot say which button
        /// it is. It also has a leaderboard consequence worth stating here, where the number is
        /// authored: <b>gem-priced land is worth nothing to a grove's score</b>. The score is
        /// the credits' worth of what is held (invariant 16g) and the server bounds the bought
        /// half by <c>earnedCredits + grantedBaseline</c> (19a), which is denominated in credits
        /// and cannot see a gem — so counting it would either overstate a forgery or understate
        /// an honest buyer, and <c>grove.ts</c> calls the second one a bug. It falls out rather
        /// than being enforced: a gem region's <see cref="cost"/> is zero, and every worth sum
        /// on both sides already skips a region that costs nothing.
        /// </para>
        /// </summary>
        public int gems;

        /// <summary>
        /// Which rung of the ladder this region is, counting from 1. Zero on starter land, which
        /// is not on the ladder because it is not sold.
        ///
        /// <para>
        /// <b>The ladder is authored and never derived from price.</b> Two reasons, and the
        /// first is that it cannot be derived: half the floor is priced in credits and half in
        /// gems, and "cheaper" across two currencies is a question with no answer. The second is
        /// the one that would have bitten later — a price is content and will be retuned, and a
        /// ladder derived from price silently reorders itself when it is, so a player part-way
        /// up would find the stretch they were saving for had moved behind one they had already
        /// passed. <c>ContentValidation</c> proves the rungs are 1..N with no gaps and no ties.
        /// </para>
        /// </summary>
        public int order;
    }
    ...
    public sealed class HomesteadPieceDto
    {
        ...
        /// <summary>
        /// The tiles this piece stands on, facing the way it was drawn. Zero or absent means
        /// one, which is what every piece was before footprints existed. See
        /// <c>GroveFootprint</c> for what a turned piece covers.
        /// </summary>
        public int cols;
        public int rows;

        /// <summary>
        /// The art's pixel size, written by <c>Tools/grove_art_facts.py</c> and checked against
        /// the PNG. Zero means unknown and the loaded sprite is measured instead — see
        /// <c>HomesteadPiece.ArtWidth</c> for the bug that makes authoring it worth the field.
        /// </summary>
        public int w;
        public int h;

        /// <summary>
        /// How many ways this piece may be turned: 1, or 4. Zero or absent means one, which is
        /// what every piece was before it could be turned. Four means the art is a folder of
        /// four renders rather than a single sprite, so a piece may not be both this and
        /// <c>animated</c>. See <c>HomesteadPiece.Facings</c>.
        /// </summary>
        public int facings;

        /// <summary>
        /// Where the picture is inside each facing's rectangle: hexadecimal, row-major from the
        /// top, one entry per facing and in facing order. Generated by
        /// <c>Tools/import_grove_art.py</c> from the rendered PNGs, never typed. See
        /// <c>GroveHitMask</c>.
        ///
        /// <para>
        /// One per facing rather than one mirrored at read time, because a facing is a
        /// different picture: a cart from the front and the same cart from the side cover
        /// different parts of one rectangle. The single <c>hit</c> below is the shape written
        /// before facings existed; it is read as facing 0 when <c>hits</c> is absent.
        /// </para>
        /// </summary>
        public string[] hits;

        /// <summary>Superseded by <c>hits</c>; read as the only facing's mask when present.</summary>
        public string hit;
