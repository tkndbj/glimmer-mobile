...
namespace GlimmerGrove.Progression
{
    ...
    public readonly struct AvatarDefinition
    {
        ...
        /// when it has them, the portrait otherwise — and where inside that rectangle the
        /// paint is. Generated by <c>Tools/grove_art_facts.py</c> into the manifest, for the
        /// reason <c>HomesteadPiece.ArtWidth</c> gives: a resident laid out before its art
        /// arrives is exactly the size it will be, and a tap on it tests the creature rather
        /// than the air around it. Zero and empty when the manifest predates them.
        /// </summary>
        public readonly int GroveArtWidth, GroveArtHeight;

        /// <summary>The hit mask as authored. Read by <c>GroveResidents</c>; see <c>GroveHitMask</c>.</summary>
        public readonly string GroveHit;

        public AvatarDefinition(string id, string portrait, string animated, int unlockLevel,
                                int unlockCost = 0, int groveArtWidth = 0, int groveArtHeight = 0,
                                string groveHit = null)
        {
            Id = id;
            ...
            GroveArtWidth = groveArtWidth < 0 ? 0 : groveArtWidth;
            GroveArtHeight = groveArtHeight < 0 ? 0 : groveArtHeight;
            GroveHit = groveHit ?? string.Empty;
        }
