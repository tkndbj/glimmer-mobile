...
namespace GlimmerGrove
{
    ...
    public sealed class SplashScreen : View
    {
        ...
        IEnumerator Run()
        {
            ...
            var jobs = Preload.Jobs();
            int done = 0;
            const int Batch = 6;

            for (int i = 0; i < jobs.Count; i += Batch)
            {
                var ops = new List<ResourceRequest>(Batch);
                for (int k = i; k < Mathf.Min(i + Batch, jobs.Count); k++)
                {
                    var op = Resources.LoadAsync(jobs[k]);
                ...
                {
                    pending = false;
                    for (int k = 0; k < ops.Count; k++) if (!ops[k].isDone) { pending = true; break; }
                    yield return null;
                }
                done += ops.Count;
                _target = Mathf.Clamp01(done / (float)jobs.Count * .82f);
            }
            ...
            {
                shapes[i]();
                _target = .82f + .18f * ((i + 1) / (float)shapes.Count);
    ...
    /// <summary>Everything the splash pulls into memory before the menu appears.</summary>
    public static class Preload
    {
        public static List<string> Jobs()
        {
            ...
            for (int i = 1; i <= 5; i++)
                for (int f = 0; f < 20; f++)
                    list.Add($"Art/Critters/c{i}/f{f:00}");
            for (int f = 0; f < 40; f++) list.Add($"Art/Fx/Victory/f{f:00}");
            for (int f = 0; f < 6; f++) list.Add($"Art/Ui/Coin/f{f}");

            string[] bg = { "grove_far", "grove_near", "grove_light", "splash_far",
                            "home_sky", "home_ground", "home_deco",
                            "map_sky", "map_ground", "map_deco", "play_0", "play_1", "play_2" };
            foreach (var b in bg) list.Add("Art/Bg/" + b);
            ...
            foreach (var u in ui) list.Add("Art/Ui/" + u);

            string[] map =
            {
                "strip0", "strip1", "strip2", "node_open", "node_lock",
                "node_s0", "node_s1", "node_s2", "node_s3", "pointer",
                "rock_grass", "rock_tall", "rock_wide", "rock_chip", "rock_plain",
                "rock_sand", "rock_palm", "rock_wood",
                "palm", "boulder", "stump", "boat", "post",
            };
            foreach (var m in map) list.Add("Art/Map/" + m);
            ...
            foreach (var s in sfx) list.Add("Audio/Sfx/" + s);

            list.Add("Fonts/GameFont");
            return list;
        }
